...
- Queue script `experiments/gspo/queue_learned_verifier_vibe_canaries.sh` waits until current GSPO jobs finish, then runs 4-generation and 8-generation canaries for `learned_verifier_vibe_2511` from the Vibe checkpoint with the hard verifier.
...
- First full learned-verifier-on-Vibe checkpoint: `checkpoint-56` eval reward `0.25755`, eval loss `0.02497`, eval runtime `762.2s`. This is above the learned-verifier-on-Vibe canary trainer eval reward `0.24287`, so keep the full run alive and compare final external metrics once available.
- The Vibe phase-2 continuation `outputs/gspo_full_contenders/vibethinker_2511_phase2_from112_eval112_20260521-0909` is also still running. It has reached `checkpoint-896`; phase-2 eval reward has risen to `0.33324`, but this reward is not directly comparable with the learned-verifier reward. Compare full runs with final external metrics and qualitative samples.
