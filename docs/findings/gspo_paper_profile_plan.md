...
- Prediction-level audit across the standing best, low-LR, conservative-temperature, and verifier-v3 canaries confirms that later variants mostly trade one reference-sensitive phrasing for another. They occasionally fix literal cases, for example `Todos los días me grita` where low-LR exactly matches `Sapa punchawmi qapariwan`, but also introduce clear regressions such as Spanish leakage in `Señora, ñawpaqman risunchik`, unnecessary stem changes around `Colegiopim warmayqa...`, or paraphrases that miss a concise reference. This supports stopping simple continuation sweeps and focusing next on data/label improvements or a small reranker over generated candidates.

Update as of 2026-05-22 05:55 UTC:

- Added `scripts/rerank_candidate_predictions.py` to measure whether multi-sample generation contains better candidates than the first/greedy-style decoded output. The oracle selector uses held-out references, so it is not deployable; it is a headroom diagnostic for selector/reward work.
- K=8 sampled candidates from the standing best adapter were generated at `outputs/rerank_candidate_evals/20260522-current-best-k8` with `temperature=0.85`, `top_p=0.95`, and `--num-return-sequences 8`. Aggregate sampled candidates were worse than greedy-like current best, but the oracle upper bound was strong. First candidate metrics: selection `22.1104`, chrF++ `35.9965`, BLEU `6.9190`, token F1 `21.2375`, source-copy `3.0877%`, leakage `0.1582%`, TER `99.0783`. Oracle-selected metrics: selection `34.0593`, chrF++ `48.4914`, BLEU `13.9889`, token F1 `36.3623`, source-copy `2.5588%`, exact-copy `0.6329%`, leakage `0.0%`, TER `76.0369`. Oracle chose a non-first candidate for `76.58%` of rows.
- Interpretation: the model already samples substantially better translations from the current distribution, but the default selection/training signal fails to consistently pick them. This is stronger evidence for training a selector/reranker or pairwise preference model than for another simple LoRA rank/LR continuation.
- Added reward profile `reference_rerank_vibe_v1` plus `experiments/gspo/run_reference_rerank_canary.sh`. It mirrors the oracle score with chrF++/BLEU/token-F1/length plus copy/leakage/artifact/repetition penalties and uses eight generations with Vibe diversity.
...
Update as of 2026-05-22 07:55 UTC:

- Added `scripts/evaluate_translation_baseline.py` and evaluated external baselines on the same clean Chanka validation split. NLLB distilled 600M (`facebook/nllb-200-distilled-600M`, `spa_Latn -> quy_Latn`) completed full eval with chrF++ `24.1377`, BLEU `0.9596`, token F1 `10.1348`, source-copy `6.2395%`, exact-copy `2.5316%`, Spanish leakage `1.2342%`, TER `166.1290`. This is much worse than the standing best adapter (`26.3808` selection, chrF++ `40.9703`, BLEU `8.1555`, token F1 `26.6169`).
- TranslateGemma 4B IT was initially gated but became accessible after HF authentication. However, it does not appear usable for Quechua/Chanka in this setup: a 5-row `es -> qu` smoke produced Devanagari/Nepali-like outputs and token F1 `0.0`. Do not spend a full eval on it unless a verified Quechua language code/template is found.
- T5Gemma remains gated with the current HF access. It needs separate access acceptance before it can be evaluated.
- Added `scripts/mbr_candidate_predictions.py`, a reference-free MBR reranker inspired by the MBR papers. It selects the candidate most similar to other sampled candidates, then applies copy/leakage/artifact/repetition guards. On the held-out K=8 samples from the current best adapter, MBR improved first-candidate selection from `22.1104` to `25.0776`, chrF++ `35.9965 -> 40.1105`, BLEU `6.9190 -> 7.0165`, token F1 `21.2375 -> 23.9195`, source-copy `3.0877% -> 2.1323%`, TER `99.0783 -> 96.7742`. It is still below the standing greedy adapter eval (`26.3808`) and far below oracle (`34.0593`), but it is the first deployable reranking method that improves K-sampled outputs.
- Paper takeaways from the newly supplied PDFs:
  - Chasing COMET / MBR self-improvement: generate candidate translations, rerank with a quality utility, then self-train on selected candidates. For us, replace COMET with MBR + Chanka lexical/anti-copy utilities because no Chanka-capable COMET is available.
  - Understanding MBR: MBR can reduce copy noise and hallucination from mode-seeking decoding, but inherits utility-metric biases. Our MBR result matches that: less source copy and better chrF++/token-F1, but not enough BLEU/TER improvement to beat the standing greedy adapter.
...
Update as of 2026-05-22 12:05 UTC:

- Ran a focused MBR candidate-pool sweep from the standing best adapter to test whether MBR improves with larger/lower-temperature candidate sets before building MBR self-training. No code changes were needed; this used `scripts/evaluate_gspo_checkpoint.py` for sampled candidates and `scripts/mbr_candidate_predictions.py` for reference-free selection.
- Conservative K=16 pool: `outputs/rerank_candidate_evals/20260522-current-best-k16-t065-p090`, `temperature=0.65`, `top_p=0.90`, 16 candidates per eval row. Aggregate candidates scored chrF++ `38.1466`, BLEU `7.5162`, token F1 `22.2791`. MBR selection improved over previous K=8 MBR but still did not beat the standing adapter or terminology-prompt eval: selection `25.8597`, chrF++ `40.8152`, BLEU `8.1692`, token F1 `24.1345`, source-copy `1.9673%`, leakage `0.1582%`, TER `93.5484`. Oracle on the same pool was much higher: selection `36.7495`, chrF++ `50.8698`, BLEU `18.9527`, token F1 `39.5376`, TER `72.1198`.
- Tighter K=16 pool: `outputs/rerank_candidate_evals/20260522-current-best-k16-t050-p085`, `temperature=0.50`, `top_p=0.85`. MBR was slightly worse overall than the conservative pool: selection `25.7385`, chrF++ `40.9420`, BLEU `5.6074`, token F1 `25.3838`, source-copy `2.1941%`, leakage `0.3165%`, TER `91.0138`. Oracle stayed high: selection `34.9225`, chrF++ `49.6340`, BLEU `17.0021`, token F1 `36.2909`, TER `75.3456`.
- Interpretation: increasing K and lowering temperature helps MBR compared with the original K=8 sample, but reference-free MBR still tops out below the unchanged adapter with terminology prompting (`selection 26.4615`). The important signal is the oracle: K=16 conservative sampling contains chrF++ `50.87` / BLEU `18.95` candidates. The remaining bottleneck is a better selector or self-training signal, not raw model diversity. MBR self-training is still plausible, but it needs a custom-data SFT path and should probably train from MBR-selected train candidates plus clean references, then evaluate reload quality.
