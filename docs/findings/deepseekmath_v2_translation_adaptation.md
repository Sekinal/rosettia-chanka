...
The first DeepSeek V4 Pro chain now follows the same evidence protocol at both training stages. After the frontier-thinking SFT head-start, it writes an `sft_seed` `cycle_manifest.json` under `${THINKING_SFT_OUTPUT_DIR}` with SFT-only metrics, predictions, and mined hardcases. If the SFT gate allows GSPO, it then writes `promotion_gate.json`, compares the GSPO metrics against the SFT-only eval metrics, mines GSPO hardcases, and writes an `initial_gspo` `cycle_manifest.json`. `REQUIRE_GSPO_PROMOTION=false` by default, so a failed first policy still produces useful hardcase data and an audit manifest.
