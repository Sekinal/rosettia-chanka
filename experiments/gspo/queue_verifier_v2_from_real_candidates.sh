...
BASELINE_EVAL_DIR="${BASELINE_EVAL_DIR:-outputs/gspo_checkpoint_evals/20260521-eosfix-ranking}"
...
while [[ ! -f "$FULL_EVAL_DIR/summary.jsonl" ]]; do
  date -u
  echo "Waiting for full checkpoint eval summary: $FULL_EVAL_DIR/summary.jsonl"
  sleep "$POLL_SECONDS"
done

mapfile -t candidate_jsonl < <(
  find "$BASELINE_EVAL_DIR" "$FULL_EVAL_DIR" -maxdepth 1 -type f -name '*_predictions.jsonl' 2>/dev/null | sort
)

if [[ "${#candidate_jsonl[@]}" -eq 0 ]]; then
  echo "No candidate prediction JSONL files found." >&2
  exit 1
fi
...
python scripts/train_verifier_chanka_unsloth.py \
  ...
  "${candidate_args[@]}" \
  --candidate-max-examples "${CANDIDATE_MAX_EXAMPLES:-4000}" \
  --num-train-epochs "${VERIFIER_NUM_TRAIN_EPOCHS:-4}" \
...
if not first:
    raise SystemExit("empty checkpoint summary")
print(json.loads(first)["adapter_path"])
PY
)"
...
eval_dir="outputs/gspo_checkpoint_evals/${STAMP}-verifier-v2-canary"
mkdir -p "$eval_dir"
python scripts/evaluate_gspo_checkpoint.py \
  --adapter-path "$CANARY_OUTPUT_DIR/chanka_gspo/final_gspo_lora" \
  --output-json "$eval_dir/verifier_v2_real_candidates_canary_metrics.json" \
  --predictions-jsonl "$eval_dir/verifier_v2_real_candidates_canary_predictions.jsonl" \
  --batch-size 1
