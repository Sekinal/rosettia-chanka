...
CANARY_OUTPUT_DIR="${CANARY_OUTPUT_DIR:-outputs/gspo_paper_profiles/2511_bleu_margin_current_best_${STAMP}}"
EVAL_DIR="${EVAL_DIR:-outputs/gspo_checkpoint_evals/${STAMP}-bleu-margin-canary}"
...
env \
  ROOT_DIR="$ROOT_DIR" \
  GSPO_REWARD_PROFILE=learned_verifier_bleu_margin_vibe_2511 \
  ...
  LEARNING_RATE="${GSPO_LEARNING_RATE:-5e-7}" \
  ...
  TEMPERATURE="${GSPO_TEMPERATURE:-0.72}" \
